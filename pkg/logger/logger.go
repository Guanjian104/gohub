...
func Debug(moduleName string, field ...zap.Field) {
	Logger.Debug(moduleName, field...)
}

// Info 告知类日志
func Info(moduleName string, field ...zap.Field) {
	Logger.Info(moduleName, field...)
}

// Warn 警告类
func Warn(moduleName string, field ...zap.Field) {
	Logger.Warn(moduleName, field...)
}

// Error 错误时记录，不应该中断程序，查看日志时重点关注
func Error(moduleName string, field ...zap.Field) {
	Logger.Error(moduleName, field...)
}

// Fatal 级别同 Error(), 写完 log 后调用 os.Exit(1) 退出程序
func Fatal(moduleName string, field ...zap.Field) {
	Logger.Fatal(moduleName, field...)
}
